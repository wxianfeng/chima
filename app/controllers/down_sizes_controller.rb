class DownSizesController < ApplicationController
  # GET /down_sizes
  # GET /down_sizes.json
  def index
  ...
  def create
    ...
    respond_to do |format|
      if @down_size.save
        format.html { redirect_to @down_size, notice: 'Down size was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @down_size.update_attributes(params[:down_size])
        format.html { redirect_to @down_size, notice: 'Down size was successfully updated.' }
        format.json { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to down_sizes_url }
      format.json { head :ok }
    end
  end
end
