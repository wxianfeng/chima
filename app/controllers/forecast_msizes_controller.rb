class ForecastMsizesController < ApplicationController
  # GET /forecast_msizes
  ...
  def create
    ...
    respond_to do |format|
      if @forecast_msize.save
        format.html { redirect_to @forecast_msize, notice: 'Forecast msize was successfully created.' }
        format.json { render json: @forecast_msize, status: :created, location: @forecast_msize }
      else
  ...
  def update
    ...
    respond_to do |format|
      if @forecast_msize.update_attributes(params[:forecast_msize])
        format.html { redirect_to @forecast_msize, notice: 'Forecast msize was successfully updated.' }
        format.json { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to forecast_msizes_url }
      format.json { head :ok }
    end
  end
end
