class UpSizesController < ApplicationController
  # GET /up_sizes
  # GET /up_sizes.json
  def index
    @up_sizes = UpSize.all
  ...
  def create
    ...
    respond_to do |format|
      if @up_size.save
        format.html { redirect_to @up_size, notice: 'Up size was successfully created.' }
  ...
  def update
    ...
    respond_to do |format|
      if @up_size.update_attributes(params[:up_size])
        format.html { redirect_to @up_size, notice: 'Up size was successfully updated.' }
        format.json { head :ok }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to up_sizes_url }
      format.json { head :ok }
    end
  end
